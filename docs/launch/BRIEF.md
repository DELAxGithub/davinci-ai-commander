...
## Dela 確定待ち項目（Antigravity はここを推測で埋めず、ドラフトに `[要確定]` プレースホルダで残す）

1. **ターゲット 1 文** — 「〇〇な人のための××アプリ」を 1 文で確定
   - 候補軸: 「DaVinci Resolve で繰り返し作業に消耗してる映像エディター」「自動化したいけど Python は書きたくない人」「企画/構成段階で bin を量産する番組制作スタッフ」
2. **開発動機 1 段落** — なぜ作ったか（Dela の原体験）
3. **価格確定** — $19 / $24 / $29 のどれ
4. **アプリ名最終確定** — DaVinci 商標問題を踏まえ、現名のままか改名か
5. **note 透明性ブロックの「なぜ無料じゃないか」** — Dela の事情を 1 段落
...
| 1. 原体験ストーリー | 既存ツール不満 → 自分の痛み → 気づき | Dela の動機（要確定） |
| 2. プロダクト提示 | 名前・価格・課金モデル即提示 | `DaVinci AI Commander` / $XX 買い切り / Gumroad |
| 3. 透明性ブロック | 「なぜ無料じゃないか」先回り告白 | Gemini API key はユーザー持ち、Dela 側コストはゼロ、それでも有料な理由（要確定） |
